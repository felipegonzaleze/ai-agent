...
def run_python_file(working_directory, file_path, args=[]):
    ...
    if not abs_target_file.startswith(abs_working_dir): return f'Error: Cannot execute "{file_path}" as it is outside the permitted working directory'
    if not os.path.exists(abs_target_file): return f'Error: File "{file_path}" not found.'
    if not file_path.endswith('.py'): return f'Error: "{file_path}" is not a Python file.'

    try: 
        commands = ["python", abs_target_file]
        if args:
        ...
        if result.returncode != 0: output.append(f"Process exited with code {result.returncode}")
